...
class StockMove(models.Model):
    ...
    def _read_group_select(self, table, aggregate_spec):
        if aggregate_spec in ('remaining_qty:sum', 'remaining_value:sum', 'remaining_value:sum_currency', 'value:sum', 'value:sum_currency'):
    ...
    def _read_group_postprocess_aggregate(self, aggregate_spec, raw_values):
        ...
        if aggregate_spec in ('value:sum', 'value:sum_currency'):
            ...
            if op == 'sum':
                return (
                    sum(move.value if move.is_in else -move.value for move in records)
            ...
            if op == 'sum_currency':
                ...
                for records in column:
                    ...
                    for record in records:
                        to_currency = record.company_currency_id._convert(
                            from_amount=record[field_name],
                            to_currency=self.env.company.currency_id)
                        if record.is_in:
                            value += to_currency
                ...
                return (x for x in values_to_return)
