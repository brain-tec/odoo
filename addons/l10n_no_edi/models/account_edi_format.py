...
class AccountEdiFormat(models.Model):
    ...
    def _bis3_get_extra_partner_domains(self, tree):
        if self.code == 'ehf_3':
            ...
            if endpoint is not None:
                ...
                if scheme == '0192' and endpoint.text:
                    return [('l10n_no_bronnoysund_number', '=', endpoint.text)]
    ...
    def _check_move_configuration(self, invoice):
        ...
        if supplier.country_code == 'NO' and not supplier.l10n_no_bronnoysund_number:
            errors.append(_("The supplier %s must have a Bronnoysund company registry.", supplier.display_name))

        if supplier.country_code != 'NO' and supplier.country_code not in COUNTRY_EAS:
            errors.append(_("The supplier %s is from a country that is not supported for EHF (Bis3)", supplier.display_name))

        customer = invoice.commercial_partner_id
        if customer.country_code == 'NO' and not customer.l10n_no_bronnoysund_number:
            errors.append(_("The customer %s must have a Bronnoysund company registry.", customer.display_name))

        if customer.country_code != 'NO' and customer.country_code not in COUNTRY_EAS:
            errors.append(_("The customer %s is from a country that is not supported for EHF (Bis3)", customer.display_name))
