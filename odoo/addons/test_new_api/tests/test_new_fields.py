...
class TestX2many(common.TransactionCase):
    ...
    def test_sudo_commands(self):
        ...
        for Partner, my_partner in [
            ...
            with self.assertRaisesRegex(AccessError, "ot allowed to create documents of type 'Users'"):
                Partner.create({
                    'name': 'foo',
                    'user_ids': [(0, 0, {
                        'login': 'foo',
            ...
            with self.assertRaisesRegex(AccessError, "not allowed to modify documents of type 'Users'"):
                my_partner.write({
            ...
            with self.assertRaisesRegex(AccessError, "not allowed to delete documents of type 'Users'"):
                my_partner.write({
        ...
        for User, my_user in [
            ...
            # 2.0 Command.CREATE
            # Case: a public/portal user creating a new users with arbitrary values
            with self.assertRaisesRegex(AccessError, "not allowed to create documents of type 'test_new_api.group'"):
                User.create({
                    'name': 'foo',
                    'group_ids': [(0, 0, {})],
                })
            # 2.1 Command.UPDATE
            # Case: a public/portal updating his user to add himself a group
            with self.assertRaisesRegex(AccessError, "not allowed to modify documents of type 'test_new_api.group'"):
                my_user.write({
                    'group_ids': [(1, my_user.group_ids[0].id, {})],
                })
            # 2.2 Command.DELETE
            # Case: a public user deleting the public user to mess with the database
            with self.assertRaisesRegex(AccessError, "not allowed to delete documents of type 'test_new_api.group'"):
                my_user.write({
