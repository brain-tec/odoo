...
class TestExpression(SavepointCaseWithUserDemo):
    ...
    def test_invalid(self):
        ...
        self.assertEqual(cm.exception.args[0], "Invalid field res.country.does_not_exist in leaf ('does_not_exist', '=', 'foo')")

        with self.assertRaises(KeyError) as cm:
            Country.search([]).filtered_domain([('does_not_exist', '=', 'foo')])
        self.assertEqual(cm.exception.args[0], 'does_not_exist')

        with self.assertRaises(ValueError) as cm:
            Country.search([('create_date', '>>', 'foo')])
        self.assertEqual(cm.exception.args[0], "Invalid leaf ('create_date', '>>', 'foo')")

        with self.assertRaises(ValueError) as cm:
            Country.search([]).filtered_domain([('create_date', '>>', 'foo')])
        self.assertEqual(cm.exception.args[0], r"stray % in format '%'")

        with self.assertRaises(psycopg2.DataError) as cm:
            Country.search([('create_date', '=', "1970-01-01'); --")])
        self.assertIn("invalid input syntax", cm.exception.args[0])
